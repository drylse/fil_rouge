...
setwd("~/IODAA/Cours_IODAA/Fil_rouge/test_opusm/Compil")
df0 <- read.csv("~/IODAA/Cours_IODAA/Fil_rouge/test_opusm/data_pretraitee/data_categorielle.csv", sep=";")
#head(df0)
df1=df0[, c(4:9,15,17,18,20,22,23)]
head(df1)
dp=data.frame(df1[,c(12)])
colnames(dp) <- c("doublepic")
dp$doublepic <-"True"
head(dp)
df2 <-cbind(df1, dp)
head(df2)
df2$doublepic[df2$Pic_J45=="False"]<- "False"
df2$doublepic[df2$Pic_J182=="False"]<- "False" 
df2$Ajout_phyto_30J[df2$Ajout_phyto_30J==0] <- "False"
df2$Ajout_phyto_30J[df2$Ajout_phyto_30J==1] <- "True"
df2$Ajout_ferti_30J[df2$Ajout_ferti_30J==0] <- "False"
df2$Ajout_ferti_30J[df2$Ajout_ferti_30J==1] <- "True"
print(df2)
df3= df2[, c(-11, -12)]
print(df3)
df=df3[(df3$doublepic=="True"),]
df[,1]=as.factor(df[,1])
df[,2]=as.factor(df[,2])
df[,3]=as.factor(df[,3])
df[,4]=as.factor(df[,4])
...
df=df[, c(-11)]
print(df)
write.table(df, "data_categorielle_opus.csv", row.names=FALSE, sep=";")
...
```{r}
trans = read_transactions("data_categorielle_opus.csv", sep = ";", format = "transactions") 
summary(trans)
